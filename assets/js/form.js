...
const Form = (() => {
  ...
  function _bindMasks() {
    ...
    $('#telefone').on('input', function () {
      $(this).val(Utils.formatPhone($(this).val()));
    });
  }
  ...
  function _validateStep1() {
    ...
    if (!isAnonymous) {
      ...
      _setValidity('#nome',  nome.length >= 3);
      _setValidity('#cpf',   Utils.validateCPF(cpf));
      _setValidity('#email', Utils.validateEmail(email));

      if (!nome || nome.length < 3)            { valid = false; }
      if (!Utils.validateCPF(cpf))             { valid = false; }
      if (!Utils.validateEmail(email))          { valid = false; }

      if (!valid) Utils.showToast('Preencha corretamente os campos obrigatórios.', 'error');
    }
    return valid;
  }
  ...
  function getFormData() {
    return {
      ...
      telefone:  isAnonymous ? null : $.trim($('#telefone').val()),
      tipo:      $('#tipo').val(),
      orgao:     $('#orgao').val(),
